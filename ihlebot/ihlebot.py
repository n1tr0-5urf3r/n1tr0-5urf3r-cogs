...
class Ihlebot:
        ...
        if valid_ip.match(ip):
            ...
            try:
                ...
                if valid == True:
                    ...
                    if response == 0:
                        await
                        self.bot.say(ip + ' is up and responding in ' +
                                     str(duration) + 'ms.')
                    else:
                        await
                        self.bot.say(ip + ' is not reachable.')
                else:
                    await
                    self.bot.say(ip + ' is not a valid IP or Domain.')

            except socket.gaierror:
                await
                self.bot.say('Whoops! That Address cant be resolved!')
    ...
    async def pr0(self, ctx):
        ...
        await
        self.bot.say("https://img.pr0gramm.com/{}".format(item))
    ...
    async def coinflip(self, ctx, player1=None, *, player2=None):
        ...
        if player1 is None and player2 is None:
            if rng < 5:
                return await
                self.bot.say("Kopf gewinnt!")
            else:
                return await
                self.bot.say("Zahl gewinnt!")
        else:
            if rng < 5:
                return await
                self.bot.say("{} hat gewonnen!".format(player1))
            else:
                return await
                self.bot.say("{} hat gewonnen!".format(player2))
...
```""")
    ...
    async def createroles(self, ctx):
        ...
        for group_channel in group_channels:
            if group_channel not in all_roles:
                await
                self.bot.create_role(author.server, name=group_channel)
                await
                self.bot.say("Role {} created".format(group_channel))
        ...
        for channel in all_channels:
            if "übungsgruppe-" in channel.name:
                ...
                await
                self.bot.edit_channel_permissions(channel, server.default_role, everyone_perms)
                # Grant permission to role
                await
                self.bot.edit_channel_permissions(channel, role, overwrite)
                await
                self.bot.say("Granted permissions for role {} to channel {}".format(role, channel))
                await
                asyncio.sleep(1.5)
    ...
    async def gruppe(self, ctx, join_group=None):
        ...
        async def send_help():
            ...
            await
            self.bot.say("Gruppe nicht gefunden oder angegeben. Verfügbare Gruppen sind:")
            embed.set_footer(text='Bot by Fabi')
            return await
            self.bot.say(embed=embed)
        ...
        if "übungsgruppe-" in join_group:
            try:
                ...
                await
                self.bot.say("{}, du wurdest zu {} hinzugefügt".format(author.mention, join_group))
    ...
    async def gruppeverlassen(self, ctx, leave_group=None):
        ...
        async def send_help():
            ...
            role_names = []
            embed = discord.Embed(
                description="**Zugeordnete Übungsgruppen**")
            for role_name in all_roles:
                role_names.append(role_name.name)
            embed.add_field(name="Gruppen", value="\n".join(role_names))
            await
            self.bot.say("Gruppe nicht gefunden oder zugeordnet. Zugeordnete Gruppen sind:")
            embed.set_footer(text='Bot by Fabi')
            return await
            self.bot.say(embed=embed)
        ...
        try:
            ...
            await
            self.bot.remove_roles(author, role)
            await
            self.bot.say("{} du wurdest aus der Gruppe {} entfernt".format(author.mention, leave_group))
        except AttributeError:
            await
            send_help()
